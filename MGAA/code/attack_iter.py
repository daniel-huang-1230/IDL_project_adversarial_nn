...
"""
resize input images in a single batch 

mgaa_adv: option flag to indicate whether we are resizing to generate adversarial examples, False can be used for simple resizing
"""
def resize_images_np(inputs_batch, batch_shape, image_height, image_width, mgaa_adv=True):
    images = np.zeros(batch_shape)
    idx = 0
    for input_x in inputs_batch:
        if mgaa_adv:
            image = imresize(input_x, [image_height, image_width]).astype(np.float) / 255.0
            images[idx, :, :, :] = image * 2.0 - 1.0
        else:
            image = imresize(input_x, [image_height, image_width]).astype(np.float)
            images[idx, :, :, :] = image
        idx += 1
...
def generate_mgaa_adv(max_epsilon, num_iter, batch_size, image_height, image_width, inputs_batch, labels_batch, meta_train_only=False, meta_test_only=False):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            labels = np.insert(labels_batch, 0, np.zeros((batch_size)))
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            for filenames, images in load_images(FLAGS.input_dir, batch_shape):
                ...
                print("MGAA expected images shape = {}".format(images.shape))
                adv_images = images.copy()
                grad_images = np.zeros(batch_shape)
